# @xiaoliyo/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@xiaoliyo/pi-ai`.

## Installation

```bash
npm install @xiaoliyo/pi-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@xiaoliyo/pi-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
import { anthropicProvider } from "@xiaoliyo/pi-ai/providers/anthropic";
...
```typescript
declare module "@xiaoliyo/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@xiaoliyo/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@xiaoliyo/pi-agent-core";
