...
```ts
import { PiClient, type ByteTransportFactory } from "@xiaoliyo/pi-client";
...
import { createUnixTransportFactory } from "@xiaoliyo/pi-client/unix";
...
The `@xiaoliyo/pi-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@xiaoliyo/pi-client/unix` subpath.
