...
// Built-in provider defaults removed: providers are user-configured endpoints.
...
function buildFallbackModel(provider: string, modelId: string, availableModels: Model<Api>[]): Model<Api> | undefined {
	...
	// No built-in defaults: use the first model of the provider.
...
export async function findInitialModel(options: {
	...
	if (availableModels.length > 0) {
		// No built-in defaults: use the first available model.
...
export async function restoreModelFromSession(
	...
	if (availableModels.length > 0) {
		// No built-in defaults: fall back to the first available model.
