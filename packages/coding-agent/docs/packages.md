...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@xiaoliyo/pi-ai`, `@xiaoliyo/pi-agent-core`, `@xiaoliyo/pi-coding-agent`, `@xiaoliyo/pi-tui`, `typebox`.
