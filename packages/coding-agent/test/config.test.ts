...
function createNpmPrefixInstall(
	template = "pi-prefix-",
	options: { npmReportedRoot?: boolean } = {},
): { prefix: string; packageDir: string } {
	const prefix = mkdtempSync(join(tmpdir(), template));
	// getInferredNpmInstall() only recognizes `<prefix>/lib/node_modules` on every
	// platform. Tests that exercise the configured-npmCommand path skip inference and
	// must instead match what `npm --prefix <prefix> root -g` reports: `lib/node_modules`
	// on POSIX, `node_modules` on Windows.
	const root = join(
		prefix,
		options.npmReportedRoot && process.platform === "win32" ? "node_modules" : join("lib", "node_modules"),
	);
	const scopeDir = join(root, "@xiaoliyo");
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@xiaoliyo+pi-coding-agent@0.67.68\\node_modules\\@xiaoliyo\\pi-coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@xiaoliyo/pi-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @xiaoliyo/pi-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@xiaoliyo/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@xiaoliyo/pi-coding-agent")).toBe(
			"Update @xiaoliyo/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@xiaoliyo/pi-coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@xiaoliyo/pi-coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @xiaoliyo/pi-coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@xiaoliyo/pi-coding-agent", undefined, {
			packageName: "@xiaoliyo/pi-coding-agent",
			installSpec: "@xiaoliyo/pi-coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@xiaoliyo/pi-coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @xiaoliyo/pi-coding-agent@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		const { prefix } = createNpmPrefixInstall("pi-prefix-", { npmReportedRoot: true });

		const command = getSelfUpdateCommand("@xiaoliyo/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @xiaoliyo/pi-coding-agent`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@xiaoliyo\\pi-coding-agent";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@xiaoliyo/pi-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @xiaoliyo/pi-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@xiaoliyo/pi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @xiaoliyo/pi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@xiaoliyo/pi-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@xiaoliyo", "pi-coding-agent");
	...
	// chmod-based read-only directories do not block writes on Windows (the DOS
	// read-only attribute never applies to directory creation semantics).
	test.skipIf(process.platform === "win32")("does not self-update when npm install path is not writable", () => {
		const { packageDir } = createNpmPrefixInstall();
		chmodSync(packageDir, 0o500);

		expect(getSelfUpdateCommand("@xiaoliyo/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@xiaoliyo/pi-coding-agent")).toContain(
